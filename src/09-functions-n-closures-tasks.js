...
function getPolynom() {
  ...
  return (arguments.length === 0) ? null : (x) => arr.reduce((accum, current, i) => accum + current
   * Math.pow(x, i));
}
...
function partialUsingArguments(fn, ...args1) {
  const arg1 = Array.from(args1);
  return function qq(...args2) {
    const arg2 = Array.from(args2);
