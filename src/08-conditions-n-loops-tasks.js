...
function getFizzBuzz(num) {
  let result = num;
  if (!(num % 3) && (num % 5)) result = 'Fizz';
  if (!(num % 5) && (num % 3)) result = 'Buzz';
...
function doRectanglesOverlap(rect1, rect2) {
  const arr1 = [rect1.top + rect1.height, rect1.left + rect1.width];
  return (arr1[0] > rect2.top && arr1[1] > rect2.left);
}
...
function isInsideCircle(circle, point) {
  return (Math.hypot(Math.abs(circle.center.x - point.x), Math.abs(circle.center.y - point.y))
  < circle.radius);
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const buffer = arr[i];
    arr.splice(i, 1);
    if (arr.indexOf(buffer) === -1) {
      result = buffer;
      break;
    }
    arr.splice(i, 0, buffer);
  }
  return result;
}
...
function isBracketsBalanced(str) {
  function isTrue(elem1, elem2) {
    let result = false;
    if (elem1 === '[' && elem2 === ']') result = true;
    if (elem1 === '(' && elem2 === ')') result = true;
    if (elem1 === '{' && elem2 === '}') result = true;
    if (elem1 === '<' && elem2 === '>') result = true;
    return result;
...
function getCommonDirectoryPath(pathes) {
  ...
  if (commonStr === '' || commonStr[commonStr.length - 1] === '/') {
    result += commonStr;
  } else if (slashInd !== null) {
    result = commonStr.substring(0, slashInd + 1);
  }
  return result;
}
...
function evaluateTicTacToePosition(position) {
  ...
  if (position[2][0] === position[1][1]
    && position[2][0] === position[0][2]) result = position[2][0];
  return result;
}
